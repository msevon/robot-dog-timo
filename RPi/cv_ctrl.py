...
class OpencvFuncs():
    ...
    def cv_detect_faces(self, img):
        ...
        if len(faces):
            ...
            for (x,y,w,h) in faces:
                cv2.rectangle(overlay_bgr,(x,y),(x+w,y+h),(64,128,255),1)
                face_area = w * h
        ...
        cv2.putText(overlay_bgr, 'NUMBER: {}'.format(len(faces)), (center_x+50, center_y+40), 
                                                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(overlay_bgr, 'ITERATE: {}'.format(self.track_faces_iterate), (center_x+50, center_y+60), 
                                                         cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(overlay_bgr, ' SPD_R: {}'.format(self.track_spd_rate), (center_x+50, center_y+80), 
                                                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(overlay_bgr, ' ACC_R: {}'.format(self.track_acc_rate), (center_x+50, center_y+100), 
                                                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    ...
    def mp_detect_hand(self, img):
        ...
        if results.multi_hand_landmarks:
            for handLms in results.multi_hand_landmarks:
                ...
                for id, lm in enumerate(handLms.landmark):
                    ...
                    cv2.circle(overlay_bgr, (cx, cy), 5, (255, 0, 0), -1)

                # draw lines
                self.mpDraw.draw_landmarks(overlay_bgr, handLms, self.mpHands.HAND_CONNECTIONS)
                ...
                if middle_finger_gs > 20 and pinky_finger_gs > 90:
                    cv2.putText(overlay_bgr, ' GS: LED Ctrl', (center_x+50, center_y+100), 
        ...
        cv2.putText(overlay_bgr, 'ITERATE: {}'.format(self.track_faces_iterate), (center_x+50, center_y+140), 
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(overlay_bgr, ' SPD_R: {}'.format(self.track_spd_rate), (center_x+50, center_y+160), 
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(overlay_bgr, ' ACC_R: {}'.format(self.track_acc_rate), (center_x+50, center_y+180), 
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        overlay_bgra = cv2.cvtColor(overlay_bgr, cv2.COLOR_BGR2BGRA)
        self.overlay = overlay_bgra
    ...
    #     cv2.putText(overlay_bgr, 'MediaPipe Pose', (100, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
    #     if results.pose_landmarks:
    #         self.mpDraw.draw_landmarks(overlay_bgr, results.pose_landmarks, self.mp_pose.POSE_CONNECTIONS)
    #     overlay_bgra = cv2.cvtColor(overlay_bgr, cv2.COLOR_BGR2BGRA)
    #     self.overlay = overlay_bgra
    ...
    def mediaPipe_faces(self, img):
        ...
        overlay_bgr = np.zeros((img.shape[0], img.shape[1], 3), dtype=np.uint8)
        cv2.putText(overlay_bgr, 'MediaPipe Faces', (100, 70),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        if results.detections:
            for detection in results.detections:
                self.mpDraw.draw_detection(overlay_bgr, detection)
        overlay_bgra = cv2.cvtColor(overlay_bgr, cv2.COLOR_BGR2BGRA)
        self.overlay = overlay_bgra
